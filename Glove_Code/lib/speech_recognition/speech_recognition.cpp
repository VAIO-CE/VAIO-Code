...
void SpeechRecognition::capture_samples(void *arg) {
  ...
  while (record_status) {
    ...
    i2s_read((i2s_port_t)1, (void *)sampleBuffer, i2s_bytes_to_read,
             &bytes_read, 100);
...
bool SpeechRecognition::microphone_inference_record(void) {
  ...
  if (inference.buf_ready == 1) {
    ei_printf("Error sample buffer overrun. Decrease the number of slices per "
              "model window "
 ...
 */
int SpeechRecognition::microphone_audio_signal_get_data(size_t offset,
                                                        size_t length,
                                                        float *out_ptr) {
